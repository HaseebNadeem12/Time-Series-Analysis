...
my_data['d1'] = diff(my_data['Inventories'],k_diff=1)
...
model = ARIMA(my_data['Inventories'],order=(1,1,1))
results = model.fit()
fcast = results.predict(len(my_data),len(my_data)+11,typ='levels').rename('ARIMA(1,1,1) Forecast')
